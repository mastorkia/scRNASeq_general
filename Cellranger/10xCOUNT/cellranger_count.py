...
'''Usage:run_cellranger.py -f <file list containing raw fastq file(s) location CIM.list>'''
#python run_cellranger.py -f file.list -c config_cellranger.yml
#cat file.list
...
def msg():
    return ''' This script runs cellranger program from 10X genomics to map the single cell reads and generate the count
                matrices. Exiting.... '''
...
def make_sample_dict():
    ...
    for fastq in fastq_list:
        ...
        #print("Directory loc: " + dir_name)
        file_name = os.path.basename(this_fastq)
        sample_plus_lane = file_name.split(".")[0]
        sample_name = sample_plus_lane.split("_S")[0]
...
def run_cellranger():
    ...
    for sample_name, dir_name in sample_dictionary.items():
        ...
        cmd = "echo " + cellranger_loc + " count --id=" + sample_name + " --sample=" + sample_name + " --transcriptome=" + this_transcriptome_loc + " --fastqs=" + dir_name + "/" \
            " --expect-cells=" + expected_cells + " | qsub -S /bin/sh -V -N " + job_name + " -l h_vmem=" + mem + " -pe smp " +  num_cores + " -e " + dir_name + "/" + " -o " +  dir_name + "/" +  " -j y -cwd"

    #cmd = cellranger_loc + " count --id=" + id_name + " --transcriptome=" + this_transcriptome_loc + " --fastqs=" + dir_name + "/" + " --expect-cells=" + expected_cells
    ...
    #generate a SLURM command
        ...
        if return_val == 0:
            print("Cellranger job successfully submitted for sample:  " + sample_name)
